#!/usr/bin/env bash
# Generate canonical CLI help reference for dotfiles scripts.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DOTFILES="$(cd "$SCRIPT_DIR/.." && pwd)"
source "$SCRIPT_DIR/../lib/common.sh"
source "$SCRIPT_DIR/../lib/output.sh" "$@"

OUTPUT_FILE="$DOTFILES/docs/reference/cli.md"
CHECK_MODE=false

usage() {
  cat <<EOF
Usage: $0 [--help] [--no-color] [--check]

Generate docs/reference/cli.md from --help output.

Options:
  --check    Exit non-zero if generated output differs from committed file.
EOF
}

parse_args() {
  show_help_if_requested usage "$@"

  while [[ $# -gt 0 ]]; do
    case "$1" in
      --check)
        CHECK_MODE=true
        shift
        ;;
      --no-color)
        shift
        ;;
      *)
        print_error "Unknown argument: $1"
        usage
        exit 1
        ;;
    esac
  done
}

append_help_block() {
  local title="$1"
  local script="$2"

  printf "## \`%s\`\n\n" "$title" >> "$TMP_FILE"
  printf '```text\n' >> "$TMP_FILE"
  if ! (cd "$DOTFILES" && bash "$script" --help) >> "$TMP_FILE" 2>&1; then
    printf '[help command failed]\n' >> "$TMP_FILE"
  fi
  printf '```\n\n' >> "$TMP_FILE"
}

main() {
  parse_args "$@"

  mkdir -p "$(dirname "$OUTPUT_FILE")"
  TMP_FILE="$(mktemp)"
  trap 'rm -f "$TMP_FILE"' EXIT

  {
    echo '# CLI Reference'
    echo ''
    echo "Generated from live \`--help\` output. Do not edit manually; run \`bash ops/generate-cli-reference.sh\`."
    echo ''
  } > "$TMP_FILE"

  append_help_block "install.sh" "install.sh"

  while IFS= read -r script; do
    # Skip launchd-internal scripts (no CLI contract)
    if grep -q '^# SCRIPT_VISIBILITY: launchd-internal' "$script"; then
      continue
    fi
    script_name="${script#"$DOTFILES"/}"
    append_help_block "$script_name" "$script_name"
  done < <(
    find "$DOTFILES/setup" "$DOTFILES/ops" "$DOTFILES/health" -type f -name '*.sh' \
      -not -path "$DOTFILES/health/checks/*" | sort
  )

  if $CHECK_MODE; then
    if cmp -s "$TMP_FILE" "$OUTPUT_FILE"; then
      print_success "CLI reference is up to date"
      exit 0
    fi

    print_error "CLI reference is stale: run bash ops/generate-cli-reference.sh"
    diff -u "$OUTPUT_FILE" "$TMP_FILE" || true
    exit 1
  fi

  mv "$TMP_FILE" "$OUTPUT_FILE"
  print_success "Generated $OUTPUT_FILE"
}

main "$@"
