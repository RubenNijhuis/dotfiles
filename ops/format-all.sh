...
done < <(find "$DOTFILES/setup" "$DOTFILES/ops" "$DOTFILES/health" "$DOTFILES/tests" "$DOTFILES/lib" "$DOTFILES/hooks" -name "*.sh" -o -name "pre-commit" -o -name "pre-push" -o -name "commit-msg" 2>/dev/null)
