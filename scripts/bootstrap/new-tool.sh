#!/usr/bin/env bash
# Scaffold a new stow package for a tool.
set -euo pipefail
...
main() {
  ...
  local stow_dir="$DOTFILES/stow/$TOOL_NAME"
  ...
  if $CONFIG_DIR; then
    ...
    print_success "Created stow/$TOOL_NAME/.config/$TOOL_NAME/"
  else
    mkdir -p "$stow_dir"
    print_success "Created stow/$TOOL_NAME/"
  fi
