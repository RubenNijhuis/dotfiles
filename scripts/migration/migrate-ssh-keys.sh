...
main() {
  ...
  if [[ -f ~/.ssh/id_ed25519 ]]; then
    ...
    if confirm "Proceed with migration? [y/N] " "N"; then
      ...
      print_indent "1. Update GitHub/GitLab with the same public key (no change needed)"
      print_indent "2. Run: ssh-add --apple-use-keychain ~/.ssh/id_ed25519_personal"
      print_indent "3. Test: ssh -T git@github.com"
    else
