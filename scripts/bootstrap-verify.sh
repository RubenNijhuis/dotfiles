...
main() {
  ...
  run_step "Keychain requirement checks" bash "$DOTFILES/scripts/check-keychain.sh" --no-color
  run_step "Docs sync check" bash "$DOTFILES/scripts/generate-cli-reference.sh" --check

  if $RUN_DOCTOR; then
    run_step "Doctor quick check" bash "$DOTFILES/scripts/doctor.sh" --quick --no-color
  else
