...
#   parse_standard_args usage --accept-yes "$@"                         # also accept --yes
#   parse_standard_args usage --accept-dry-run --accept-yes "$@"        # both
parse_standard_args() {
  local usage_fn="$1"; shift

  local accept_dry_run=false accept_yes=false
  while [[ $# -gt 0 ]]; do
    case "${1:-}" in
      --accept-dry-run) accept_dry_run=true; shift ;;
      --accept-yes)     accept_yes=true;     shift ;;
      *) break ;;
    esac
  done
